...
export function Shiori({ url, image, title, description }: Props) {
	return (
		...
		>
			<div className="-mx-4 -mt-2 w-44 h-16 ichimatsu" />
			<div className="-mx-4 w-44 h-32 grid place-items-center">
				{image && (
					<img src={image} alt={`${url} thumbnail`} className="max-h-24" />
				)}
			</div>
			<div className="grid place-items-center border-y border-emerald-900">
				<h2 className="max-w-36 text-emerald-900 text-xl text-center text-balance break-words leading-7 font-bold line-clamp-3">
					{title}
				</h2>
			</div>
			{description ? (
